...
router.post('/search', isLogin, async (req, res, next)=>{
    try {
        ...
        const url= "https://www.nl.go.kr/NL/search/openApi/search.do?key=" + process.env.libraryKey + "&apiType=xml&pageNum=1&pageSize=20&category=%EB%8F%84%EC%84%9C&systemType=%EC%98%A8%EB%9D%BC%EC%9D%B8%EC%9E%90%EB%A3%8C&kwd=" + text;
        console.log(url);
...
router.post('/image/:isbn', isLogin, async (req, res, next)=>{
    try {
        ...
        const isbn= req.params.isbn.split(" ")[0].replace(regex, "");
            ...
            if(year && controlno) {
                ...
                request.get(img2_url, async (err, res, body)=> {
                    if(!err && res.statusCode==200) {
                        ...
                        console.log('res 코드: ', res.statusCode);
                });
            }
        });
        ...
        img1_promise.then(async (text1)=>{
            ...
            img2_promise.then(async (text2)=>{
                ...
                return res.json({ code: 200, img: img_url });
            })
        });
